...
Files.prototype.uploadFileCloud = (filePath, uploadFileObj, res) => {
    fs.readFile(filePath, (fileReadErr, fileData) => {
        ...
        uploadFileToS3(uploadFileObj.uniqFileName, fileData, (s3UploadErr, uploadedFile) => {
            if (s3UploadErr) {
            ...
            newFlile.save((fileSaveErr, savedFile) => {
                if (fileSaveErr) {
                    removeFileFromS3(uploadFileObj.uniqFileName, (removeFileS3Err, removedFileS3) => {})
                    return common.sendErrorResponse(res, 'Error in saving file entry to db');
                }
...
Files.prototype.getFileFromS3 = (fileId, res) => {
    FilesModel.findOne({_id: common.castToObjectId(fileId)}, {uniqFileName: 1}, (fileByIdErr, fileById) => {
        ...
        getFileFromS3(fileById.uniqFileName, (fileFromS3Err, fileFromS3) => {
            ...
            // return res.send({
            //     msg: 'Successfully got file from S3',
            //     file: fileFromS3.Body
            // })
        })
...
Files.prototype.deleteFileFromCloud = (fileId, res) => {
    ...
    FilesModel.findOne({_id: fileId}, {uniqFileName: 1}, (fileByIdErr, fileById) => {
        if (fileByIdErr || (fileById && !fileById.uniqFileName)) {
            return common.sendErrorResponse(res, 'Error in getting file details');
        }
        ...
        removeFileFromS3(fileById.uniqFileName, (fileFromS3Err, fileFromS3) => {
            if (fileFromS3Err) {
                return common.sendErrorResponse(res, 'Error in deleting file from S3');
            }

            FilesModel.remove({_id: fileId}, (fileRemoveErr, removedFile) => {
                if (fileRemoveErr) {
                    return common.sendErrorResponse(res, 'Error in removing file details');
                }
...
Files.prototype.getFileUrlByName = (res, uniqFileName, callback) => {
    FilesModel.findOne({uniqFileName: uniqFileName}, {type: 1, fileName: 1, uniqFileName: 1}, (fileByIdErr, fileById) => {
        if (fileByIdErr || !fileById || !fileById.uniqFileName) {
            return common.sendErrorResponse(res, 'Error in getting file details');
        }
