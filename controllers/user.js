...
UserController.prototype.findUserByEmailId = (
  emailId,
  projection,
  callback
) => {
  User.findOne({ emailId: emailId }, projection, (err, existingUser) => {
    callback(err, existingUser);
  });
};

UserController.prototype.findUserByUserId = (userId, projection, callback) => {
  User.findOne({ _id: userId }, projection, (err, existingUser) => {
    callback(err, existingUser);
  });
};
  ...
  User.find(filter, (err, data) => {
    if (data && data[0]) {
      data.forEach(function (el) {
        el.createdAt = el._id.getTimestamp();
      });
    }
    callback(err, data);
  });
...
UserController.prototype.findUserByQuery = (
  findQuery,
  projection,
  callback
) => {
  User.findOne(findQuery, projection, (err, existingUser) => {
    callback(err, existingUser);
  });
};

UserController.prototype.updateLastLoginDate = (userId) => {
  const userObj = {
    updatedAt: new Date(),
  };

  User.updateOne(
    { _id: common.castToObjectId(userId) },
    userObj,
    (updateErr, updateResult) => {}
  );
};

UserController.prototype.updateUserDetails = (req, res, userId) => {
  const userObj = {
  ...
  User.updateOne(
    ...
    (updateErr, updateResult) => {
      if (updateErr || !updateResult) {
        return common.sendErrorResponse(res, "Error in updating user details");
      }
...
UserController.prototype.searchUser = (searchQuery, callback) => {
  User.find({
    $text: {
      $search: searchQuery,
      $caseSensitive: false,
