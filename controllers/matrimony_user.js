...
MatrimonyUser.prototype.addUser = (req, res, callback) => {
    console.log(req.body.personalDetails.rashi);
    const newUser = new matrimonyUser({
        createdBy: common.getUserId(req),
        name: req.body.name,
        profilePicCount: req.files.length,
        ...
        // },
        familyDetails: JSON.parse(req.body.familyDetails),
        // {
        //     fathersName: String,
        //     mothersName: String,
...
MatrimonyUser.prototype.updateUserDetails = (req, res, callback) => {
    matrimonyUser
        .updateOne(
            { _id: req.body.userId, ownerId: common.getUserId(req) },
            {
                $set: {
                    name: req.body.name,
                    contactNumber: req.body.contactNumber,
...
MatrimonyUser.prototype.getUserByOwnerId = (req, res, callback) => {
    ...
    matrimonyUser.findOne({ ownerId: common.getUserId(req) }, (err, user) => {
        if (err || !user) {
            return common.sendErrorResponse(res, "owner dont have matrimony user");
        }
